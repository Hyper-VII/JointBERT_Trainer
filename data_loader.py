...
class NERProcessor:
    """Processor for the pd2014 ner data set """
...
def convert_examples_to_features(examples, max_seq_len, tokenizer,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        date_error_num = 0
        error_list = []
        if len(example.words) != len(example.slot_labels):
            date_error_num += 1
            error_list.append(example.words)
            break
        tokens = []
        tokens.extend(example.words)
        slot_labels_ids = []
        slot_labels_ids.extend(example.slot_labels)
